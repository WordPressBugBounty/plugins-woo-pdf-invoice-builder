...
final class RednaoWooCommercePDFInvoice
{
    ...
    public function LoadMetaScriptWhenNeeded()
    {
        ...
        if ($screen != null && ($screen->post_type == 'shop_order' || $screen->id === $order_screen_id)) {
            global $theorder;
            if ($theorder != null) {
            ...
            // For debugging, you can uncomment these lines to see the last query and error.
            ...
            wp_localize_script(
                ...
                array(
                    ...
                    'OrderId' => $orderid,
                    'CanViewOrderNonce' => wp_create_nonce('can_view_order_' . $orderid),
    ...
    public function AttachPDFEmail($attachments, $email_id, $order)
    {
        ...
        foreach ($files as $currentFile) {
            ...
            if (is_dir($currentFile) && time() - filemtime($currentFile) > 86400) {
